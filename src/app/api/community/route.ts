...
/**
 * Validates that community data has the required structure.
 */
function validateCommunityData(data: unknown): data is CommunityData {
  if (!data || typeof data !== 'object') return false;
  const d = data as Record<string, unknown>;

  if (!d.globalGoal || typeof d.globalGoal !== 'object') return false;
  if (!Array.isArray(d.challenges) || d.challenges.length !== 3) return false;
  if (!Array.isArray(d.localInitiatives) || d.localInitiatives.length < 1) return false;
...
export async function GET() {
  try {
    ...
    if (!validateCommunityData(parsedData)) {
      throw new Error('AI returned malformed community data');
    }
